...
public class ListNotificationsUseCase {
     ...
     * 创建用户通知列表查询用例。
     *
     * @param notificationPort 通知查询与保存端口
     */
    public ListNotificationsUseCase(NotificationPort notificationPort) {
        this.notificationPort = Objects.requireNonNull(notificationPort, "通知端口不能为空");
    }
    ...
    public Result list(Query query) {
        ...
        return new Result(notificationPort.listByRecipient(query.recipientId(), query.projectId(), query.read()));
    }
    ...
    public NotificationItem markRead(MarkReadCommand command) {
        ...
        NotificationItem notification = notificationPort.findById(command.notificationId())
                .orElseThrow(() -> new IllegalArgumentException("通知不存在"));
        if (!notification.recipientId().equals(command.recipientId())) {
            throw new AccessDeniedException("禁止标记他人通知");
        }
        return notificationPort.save(notification.markRead(command.readAt()));
    }
    ...
    public interface NotificationPort {
         ...
         * @return 通知列表项
         */
        List<NotificationItem> listByRecipient(Long recipientId, Optional<String> projectId, Optional<Boolean> read);
